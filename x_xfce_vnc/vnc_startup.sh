...
vncserver $DISPLAY -depth $VNC_COL_DEPTH -geometry $VNC_RESOLUTION -log *:stderr:100 -SecurityTypes VncAuth -AlwaysShared &> $STARTUPDIR/vnc_blet_startup.log
...
PID_SUB=$(cat $HOME/.vnc/$(hostname)$DISPLAY.pid)

echo -e "PID_SUB = $PID_SUB"
...
# nxproxy start
echo -e "nxproxy starting wrapper starting..."
$HOME/nxwrapperdesktop &> $HOME/nxproxy.log &
#nxproxy -S listen=1 &> $STARTUPDIR/nxproxy.log &
...
while [[ -d /proc/$PID_SUB ]]
do 
    sleep 100
done

# echo -e "we will sleep now"
# sleep infinity